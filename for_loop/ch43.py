# Ask which direction the user wants to count (up or down). If they select up, then ask them for the top number and then count from 1 to that number. If they select down, ask them to enter a number below 20 and then count down from 20 to that number. If they entered something other than up or down, display the message “I don’t understand”.

# Ask user to direction to count.
direction = input("In what direction would you like to count. (up/down): ")
direction = direction.lower()

# Condition
if direction == "up":
    topNum = int(input("Please enter the top number: "))
    # Count 1 to top number.
    for i in range(1, topNum+1):
        print(i)
elif direction == "down":
    lastNum = int(input("Please enter a number below 20: "))
    # Count down from 20 to provided number.
    for i in range(20, lastNum-1, -1):
        print(i)
else:
    print("I don't understand")
